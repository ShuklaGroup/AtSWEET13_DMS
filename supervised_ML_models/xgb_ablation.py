...
OUT_DIR="xgb_ablation_outputs"
os.makedirs(OUT_DIR,exist_ok=True)

OUTER_FOLDS=5
RANDOM_SEED=42

BASE_PARAMS={"n_estimators":600,"max_depth":5,"learning_rate":0.03,"subsample":0.8,"colsample_bytree":0.8,"min_child_weight":1,"reg_alpha":0.0,"reg_lambda":1.0}

ABLATIONS={"n_estimators":[200,400,800],"learning_rate":[0.005,0.01,0.02],"max_depth":[5,7,9]}
...
def build_xgb(params):
    return XGBRegressor(n_estimators=params["n_estimators"],max_depth=params["max_depth"],learning_rate=params["learning_rate"],subsample=params["subsample"],colsample_bytree=params["colsample_bytree"],min_child_weight=params["min_child_weight"],reg_alpha=params["reg_alpha"],reg_lambda=params["reg_lambda"],objective="reg:squarederror",tree_method="hist",n_jobs=-1,random_state=RANDOM_SEED)
...
for param_name,param_values in ABLATIONS.items():
    ...
    out_path=os.path.join(OUT_DIR,f"xgb_ablation_{param_name}.csv")
...
all_path=os.path.join(OUT_DIR,"xgb_ablation_all_results.csv")
...
best_path=os.path.join(OUT_DIR,"xgb_ablation_best_summary.csv")
