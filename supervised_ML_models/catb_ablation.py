...
OUT_DIR="catb_ablation_outputs"
os.makedirs(OUT_DIR,exist_ok=True)

OUTER_FOLDS=5
RANDOM_SEED=42

BASE_PARAMS={"iterations":1000,"depth":6,"learning_rate":0.03,"l2_leaf_reg":3.0,"bagging_temperature":1.0,"random_strength":1.0,"border_count":254}

ABLATIONS={"iterations":[300,600,1000],"learning_rate":[0.005,0.01,0.02],"depth":[8,10],"l2_leaf_reg":[10.0,30.0,50.0],"random_strength":[1.0,5.0]}
...
def build_cat(params):
    return CatBoostRegressor(iterations=params["iterations"],depth=params["depth"],learning_rate=params["learning_rate"],l2_leaf_reg=params["l2_leaf_reg"],bagging_temperature=params["bagging_temperature"],random_strength=params["random_strength"],border_count=params["border_count"],bootstrap_type="Bayesian",loss_function="RMSE",eval_metric="RMSE",verbose=False,thread_count=-1,random_seed=RANDOM_SEED)
...
for param_name,param_values in ABLATIONS.items():
    ...
    out_path=os.path.join(OUT_DIR,f"catb_ablation_{param_name}.csv")
...
all_path=os.path.join(OUT_DIR,"catb_ablation_all_results.csv")
...
best_path=os.path.join(OUT_DIR,"catb_ablation_best_summary.csv")
